...
For example, if the theme is called `gatsby-theme-foo` and it has a file located here:
...
```bash
src/gatsby-theme-foo/components/bar.js
```
...
<Aside warning>
  ...
  **NOTE:** One confusing part about component shadowing is that we have to shadow the _parent_ theme. This means that, while we’ve installed `@jlengstorf/gatsby-theme-style`, the component we’re shadowing actually comes from `@jlengstorf/gatsby-theme-data`. [We’re working on improving that.](https://github.com/gatsbyjs/gatsby/issues/12813)

</Aside>

After saving the new component, stop the site (`control` + `C`), then clear the cache[^shadowing-bug] and start it again:

```bash
# Clear the cache so the shadowed component is picked up
# see https://github.com/gatsbyjs/gatsby/issues/11456
gatsby clean
...
[^shadowing-bug]:
  In the future it won’t be necessary to clear the cache to get shadowing to work. There’s an [open issue](https://github.com/gatsbyjs/gatsby/issues/11456) tracking the fix for this. Our custom Webpack resolver currently doesn’t invalidate the path to the component when a new component is added, so we have to clear the cache to force it to look fresh.
...
- [Deep dive into the theme architecture](/code/data-abstraction-deep-dive/)
